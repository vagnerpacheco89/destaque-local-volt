(() => {
  ...
  const locationByName = {
    'Marcelo A.': 'Pedra Branca • Palhoça',
    'Camila R.': 'Pagani • Palhoça',
    'João P.': 'São José',
    'Renata S.': 'Passa Vinte • Palhoça',
    'Eduardo M.': 'Ponte do Imaruim • Palhoça',
    'Paula F.': 'Florianópolis continental'
  };

  const source = [...grid.querySelectorAll('blockquote')].map((quote) => {
    const name = quote.querySelector('footer')?.textContent?.trim() || '';
    return {
      text: quote.querySelector('p')?.textContent?.trim() || '',
      name,
      location: locationByName[name] || 'Palhoça'
    };
  ...
  const makeCard = ({ text, name, location }) => {
    ...
    personMeta.className = 'review-marquee-location';
    personMeta.textContent = location;
  ...
  const topSequence = [...source];
  const bottomSequence = [...source.slice(3), ...source.slice(0, 3)];

  grid.innerHTML = '';
  grid.classList.add('reviews-marquee');
  grid.dataset.marqueeReady = 'true';
  grid.append(
    makeRow(topSequence, 'left'),
    makeRow(bottomSequence, 'right')
  );
  ...
  style.textContent = `
    ...
    .reviews .review-marquee-row--left .review-marquee-track {
      animation:volt-review-left 64s linear infinite;
    }

    .reviews .review-marquee-row--right .review-marquee-track {
      animation:volt-review-right 68s linear infinite;
    }
    ...
    .reviews .review-marquee-location {
      color:#aeb3ae !important;
